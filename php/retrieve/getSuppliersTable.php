...
$dataTable = getSuppliersTable($conexion, $_GET);
...
for($i = 1; $i < sizeOf($dataTable); $i++) {
	$tableString.="<tr supplierId='".$dataTable[$i][0]."' id='supplier".$dataTable[$i][0]."' class='doubleClickable'>";
	for($j = 1; $j < sizeOf($dataTable[$i]); $j++) {
		$tableString.='<td align="right">'.$dataTable[$i][$j].'</td>';
	}
	$tableString.="</tr>";
}
$jsondata['objectId'] = 'suppliersTable';
